...
export const SelectionVisualize = () => {
  ...
  const selectionSort = async () => {
    ...
    for (let i = 0; i < arrCopy.length; i++) {
      let minIdx = i;
      setActiveIndices([i]);
      setDescription(`Finding the minimum element from index ${i}`);
      await sleep(speedRef.current);
      await checkPaused();

      for (let j = i + 1; j < arrCopy.length; j++) {
        setActiveIndices([i, j]);
        setDescription(`Comparing ${arrCopy[j]} with ${arrCopy[minIdx]}`);
        await sleep(speedRef.current);
        await checkPaused();
      ...
      if (minIdx !== i) {
        ...
        setArray([...arrCopy]);
        setDescription(`Swapped ${arrCopy[i]} and ${arrCopy[minIdx]}`);
        await sleep(speedRef.current);
        await checkPaused();
      }
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-white to-blue-100 p-8">
      <div className="max-w-7xl mx-auto flex flex-col gap-8">
        <h1 className="text-6xl font-bold text-center text-gray-800 mt-9 mb-4">Selection Sort Visualizer</h1>
        ...
        <div className="mt-8 bg-white border border-gray-300 p-4 rounded-lg shadow-md">
          ...
          <div className="flex justify-evenly">
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg">Current element being compared</span>
            </div>
            <div className="flex items-center gap-2">
